...
7. Advanced Design & Data Loading Guidelines (NEW)
...
LoggerService.error('Failed', exception);
...
### 12. Font Size (NO .sp)
```dart
// ✅ Preferred — use text style helpers (they handle scaling centrally)
Text(
  AppStrings.example,
...
Text(
  AppStrings.example,
  style: TextStyle(fontSize: 14.sp), // ✖️ forbidden
);
```
...
class HomeController extends GetxController {
  ...
  Future<void> loadUsers() async {
    try {
      ...
      LoggerService.error('Load users failed', error);
...
class HomeRepository {
  Future<List<User>> fetchUsers() async {
    try {
      ...
      LoggerService.error('API call failed', error);
      rethrow;
    }
  }
}
...
LoggerService.error('API failed', exception);
...
Future<void> fetchData() async {
  try {
    ...
    LoggerService.error('Fetch failed', error);
    AppSnackBar.errorSnackBar(
      title: AppStrings.error,
...
Future<void> submit() async {
  ...
  try {
    ...
    LoggerService.error('Submit failed', error);
    AppSnackBar.errorSnackBar(
      title: AppStrings.error,
...
## 🧠 Advanced Design & Data Loading Guidelines (NEW)

### 🎨 Figma MCP–Driven UI Design Rules

* UI must be designed with **Figma MCP mindset** before coding.
* Never implement an entire screen UI in a single file.
* Avoid private widgets inside screen files.
* Always extract UI blocks into reusable widgets.

**Rules:**

* Screens (`views/screens`) should only manage layout + state binding.
* UI blocks must be placed in `views/widgets`.
* If a widget exceeds ~50 lines or contains nested UI, extract it.
* Nested Columns/Rows must become nested widgets.

---

### 🔥 Firebase Progressive Data Loading Strategy

Firebase data loading must be **progressive and non-blocking**.

**Correct Flow:**

1. Load parent collection first.
2. Render parent list immediately.
3. Load child/related documents **individually** using reference IDs or user IDs.
4. Display shimmer/placeholder while child data loads.
5. Child data loads in parallel, never blocking parent UI.

**Strict Rules:**

* Never wait for multiple collections before ending loading.
* Parent data controls screen loading.
* Child-level loading must be independent.

---

### ⏳ Loading & State Separation

Different loading states must be handled separately:

1. Screen-level loading → `EasyLoading`
2. List-level loading → Shimmer widgets
3. Item-level loading → Per-item shimmer/placeholders
4. Image loading → `CachedNetworkImage`

**Rules:**

* Never use `FutureBuilder` or `StreamBuilder` directly in UI.
* Firebase calls must live in Repository layer.
* Async logic must live in Controller.

---

### 🌐 Network & Performance Rules

* Always check network connectivity before Firebase calls.
* Handle empty, error, and partial-data states.
* Never call async methods inside `build()`.
* Avoid repeated Firebase reads for the same data.

---

### 🎨 Theming Rules (Extended)

* Default theme must always be **Light Mode**.
* Dark Mode is optional and user-controlled.
* System theme must NOT auto-switch the app theme.

**Design Rules:**

* Always consider both light and dark themes during design.
* Ensure contrast, shadows, and readability in both modes.
* Use only `AppColors` and theme helpers.
* Never hardcode colors.

---

### 🌍 Localization Rules (Extended)

* Default language: English
* Secondary language: Bangla (bn)

**Rules:**

* Never hardcode text.
* All text must come from `AppStrings`.
* Layouts must support long Bangla text.
* Buttons, paddings, and widths must be flexible.
...
### 🧩 Feature Creation Enforcement

When creating a new feature, AI must generate:

1. Feature folder
2. Controller (with static instance getter)
3. Repository (Firebase logic)
4. Model (fromJson / toJson)
5. Screen (StatelessWidget)
6. Widgets folder (UI blocks)
7. Progressive loading states
8. AppStrings entries (EN + BN)
...
* Blocks UI for child Firebase data
* Hardcodes text or colors
* Ignores theming or localization

❌ It is incorrect.

Always prefer scalable, modular, progressive, theme-aware, and localization-ready implementations.
